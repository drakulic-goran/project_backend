...
public interface SubjectRepository extends CrudRepository<SubjectEntity, Integer> {
	...
	@Query("select sub from StudentEntity s join s.student_department sd join sd.classes cl join cl.clas cd join cd.subjects su join su.subject sub where s.id=:student and sub.status=1")
	public Iterable<SubjectEntity> findByStudent(Integer student);

	@Query("select new com.iktpreobuka.projekat_za_kraj.entities.dto.StudentSubjectsDto(s, sub) from ParentEntity p join p.students s join s.student_department sd join sd.classes cl join cl.clas cd join cd.subjects su join su.subject sub where p.id=:parent and sub.status=1 and cl.status=1 and su.status=1")
